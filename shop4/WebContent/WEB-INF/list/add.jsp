...
$().ready(function(){
		...
		$.ajax({
			...
			success : function(data) {
				$.each(data,function(index,n) {
					var provinceId = data[index].provinceId;
					$("#Provinces").append("<option value='"+provinceId+"'>"+ data[index].province+ "</option>");
				})
			}
		})
	})
	...
	$("#Provinces").change(function() {
		...
		$("#Countys").prop("disabled", true);
		var provinceId = $("#Provinces option:checked").val();
		$.ajax({
			url : "searchCityByProvince.do",
			type : "post",
			data : {provinceId : provinceId},
			dataType : "json",
			success : function(data) {
				$.each(data,function(index,n) {
					var cityId = data[index].cityId;
					$("#Citys").append("<option class='CITY' value='"+cityId+"'>"+ data[index].city+ "</option>");
				})
			}
		})
	})
	...
	$("#Citys").change(function() {
		$(".AREA").remove();
		$("#Countys").prop("disabled",false);
		var cityId = $("#Citys option:checked").val();
		$.ajax({
			url : "searchAreaByCity.do",
			type : "post",
			data : {cityId : cityId},
			dataType : "json",
			success : function(data) {
				$.each(data,function(index,n) {
					var areaId = data[index].areaId;
					$("#Countys").append("<option class='AREA' value='"+areaId+"'>"+ data[index].area+ "</option>");
				})
			}
		})
	})
	...
	$("#J_editAddrOk").click(function() {
		...
		var areaId = $("#Countys option:checked").val();
		var price = $("#price").data("price");
		location.href = "addAddress.do?street="+ street+ "&person="+ person+ "&phone="+ phone
				+ "&areaId="+ areaId+"&product_id="+"${product_id}"+"&price="+ "${price}";
	})
		
	 //选中地址
	$(".address").click(function() {
		$(".address").removeClass("addressConfirm");
		$(this).addClass("addressConfirm");
		$(".address").css({"border" : "1px solid #dfdfdf"});
 		$(this).css({"border" : "1px solid #ee4644"});
	})
	
	//删除地址
	$(".deleteAddress").click(function() {
		var addressId = $(this).parent().parent().data("addressid");
		location.href = "deleteAddress.do?addressId="+ addressId+"&product_id="+"${product_id}"+"&price="+ "${price}";
	})
	
	//立即下单
	$("#xiadan").click(function(){
		var addressId = $(".deleteAddress").parent().parent().data("addressid");
		location.href="showOrders.do?product_id="+"${product_id}"+"&price="+"${price}"+"&addressId="+addressId;
	})
})
</script>
</head>
	...
	<div class="pc-header-nav">
		<div class="pc-header-con">
			<div class="fl pc-header-link">
				您好！，欢迎来京东 <a href="login.html" target="_blank">请登录</a> <a
							...
							<div class="xm-box">
								...
								<div class="box-bd">
									...
									<c:forEach items="${addrList}" var="addr">
										<dl class="item address addressConfirm" data-addressid="${addr.id}">
											<dt>
												<strong class="itemConsignee">${addr.person}</strong> <span
													class="itemTag tag">家</span>
											</dt>
											<dd>
												<p class="tel itemTel">${addr.phone}</p>
												<p class="itemRegion">${addr.province} ${addr.city}</p>
												<p class="itemStreet">${addr.street}</p>
												<span class="edit-btn J_editAddr" style="display: none;">编辑</span>
												<span class="edit-btn J_editAddr deleteAddress" style="display: block;">删除地址</span>
											</dd>
										...
										<div class="item use-new-addr" id="J_useNewAddr" data-state="off">
											...
											<img src="images/add_cart.png" /></span> 使用新地址
										</div>
									</div>
...
<!-- 								</div> -->
						...
						<div class="checkout-box-ft">
									...
									<div class="box-bd">
										<dl class="checkout-goods-list">
											...
											<c:forEach items="${pro}" var="pro">
												<dd class="item clearfix product" data-productId="${pro.id}">
													<div class="item-row">
														<div class="col col-1">
															<div class="g-pic">
																<img src="${pro.imgs[0].path }" width="40" height="40" />
															</div>
															<div class="g-info">
																<a href="#" target="_blank">${pro.name}</a>
															</div>
														</div>
														<div class="col col-2">${pro.price}</div>
														<div class="col col-3">${pro.amount}</div>
														<div class="col col-4">${pro.price*pro.amount}元</div>
													</div>
												</dd>
											</c:forEach>
										</dl>
										<div class="checkout-count clearfix">
											...
											<div class="checkout-price">
												<ul>
													...
													<li>订单总额：<span data-price="${price}" id="price">${price}</span> </li>
													<li>运费：<span id="postageDesc">0元</span>
													</li>
												</ul>
												<p class="checkout-total">
													应付总额：<span><strong id="totalPrice">${price}</strong>元</span>
												</p>
											</div>
											<!--  -->
										</div>
							...
							<div class="checkout-confirm">
								...
								<a href="showCart.do" class="btn btn-lineDakeLight btn-back-cart">返回购物车</a>
								<a class="btn btn-primary" id="xiadan">立即下单</a>
