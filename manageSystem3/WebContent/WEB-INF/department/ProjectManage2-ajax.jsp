...
<script type="text/javascript">
	$().ready(function() {
				...
				$("#doDelete").click(function() {
					...
					if (becheckbox.length > 0) {
						...
						$.ajax({
							url:"department",
							type:"post",
							data:{type:"deletePro2",selectId:becheckbox,depName:"${depName}"}, 
							dataType:"text",
							success:function(data){
								if(data=="true"){
									var option = "<option value='"+becheckbox+"'>"+proName+"</option>"
				...
				$("#Add").click(function(){
					var proId = $("#departmentNo").val();
					var d_id = ${d_id};
					var proName="";
					var i = 0;
					$("#departmentNo").children().each(function(index,element){
						if($(this).val()==proId){
							proName = $(this).text();
							i = index;
						}
					...
					$.ajax({
						url:"department",
						type:"post",
						data:{type:"addProToDep2",selectId:d_id,proName:proName},
						dataType:"text",
						success:function(data){
							if(data=="true"){
								var tr = "<tr><td>"+proId+"</td><td>"+proName+"</td><td></td></tr>"
								$("#pro").append(tr);
								$("#departmentNo").children().eq(i).remove();
							}
						}
	...
	<div id="main">
		...
		<table id="pro" class="table table-striped table-bordered table-hover">
			<tr id="table-head">
				<th>ID</th>
				<th>项目</th>
				<th id="allselect"><input type="checkbox" name="duoXuan"
					id="all" class="btn checkbox" />全选/反选</th>
			</tr>
			...
			<c:forEach items="${pagevo.records}" var="pro">
				<tr data-id="${pro.id}" id="content">
					<td>${pro.id}</td>
					<td>${pro.name}</td>
					<td><input type="checkbox" name="selectId" id="check"
						value="${pro.id}" /></td>
				</tr>
			</c:forEach>
		</table>
		...
		<ul class="pagination" id="pagination">
			...
			<c:if test="${pagevo.pageNo >1}">
				<li><a
					href="${pagevo.url}type=ProjectManage&pageNo=${1}&selectId=${d_id}">首页</a></li>
				<li><a
					href="${pagevo.url}type=ProjectManage&pageNo=${pagevo.pageNo - 1}&selectId=${d_id}">上一页</a></li>
			</c:if>
				...
				<c:if test="${pagevo.pageNo>=1 && pagevo.pageNo<=3}">
					<c:forEach begin="${1 }" end="${5 }" varStatus="status">
						<li
							...
							<a
							href="${pagevo.url}type=ProjectManage&pageNo=${status.index}&selectId=${d_id}">${status.index}</a>
						</li>
					</c:forEach>
				</c:if>
			...
			<c:if test="${pagevo.pageNo < pagevo.totalPageSize}">
				<li><a
					href="${pagevo.url}type=ProjectManage&pageNo=${pagevo.pageNo + 1}&selectId=${d_id}">下一页</a></li>
				<li><a
					href="${pagevo.url}type=ProjectManage&pageNo=${pagevo.totalPageSize}&selectId=${d_id}">尾页</a></li>
			</c:if>
		</ul>
		...
		<div>
			<br>
			<div id="project">
				<label>未选择项目:</label> <select name="departmentNo" id="departmentNo">
					<c:forEach items="${noList}" var="noList">
						<option value="${noList.id}">${noList.name}</option>
					</c:forEach>
				</select>
			</div>
