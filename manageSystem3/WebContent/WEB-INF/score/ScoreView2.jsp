...
<script type="text/javascript">
	$().ready(function() {
				...
				$("#tiaozhuan").click(function(){
					...
					location.href = "score?type=manage&pageNo="+pageNum;
				})
			...
			$("#search").click(function(){
				...
				location.href = "score?type=search2&name="+name+"&grade="+grade+"&project="+project+"&department="+department+"&value="+value;
			})	
...
<body>
	<div id="main">	
		...
		<ul class="pagination" id="pagination">
			...
			<li><a href="${pagevo.url}type=search2&pageNo=${1}&name=${sco.emp.name}&grade=${sco.grade}&project=${sco.pro.name}&department=${sco.dep.name}&value=${sco.value}">首页</a></li>
			<li><a href="${pagevo.url}type=search2&pageNo=${pagevo.pageNo - 1}&name=${sco.emp.name}&grade=${sco.grade}&project=${sco.pro.name}&department=${sco.dep.name}&value=${sco.value}">上一页</a></li>
			</c:if>
			...
			<c:if test="${pagevo.totalPageSize > 5}">
				...
				<c:if test="${pagevo.pageNo > 3 && pagevo.pageNo <= pagevo.totalPageSize-2}">
					<c:forEach begin="${pagevo.pageNo-2 }" end="${pagevo.pageNo+2 }" varStatus="status">
						<li <c:if test="${pagevo.pageNo == status.index}"> class="active"</c:if> >
						<a href="${pagevo.url}type=search2&pageNo=${status.index}&name=${sco.emp.name}&grade=${sco.grade}&project=${sco.pro.name}&department=${sco.dep.name}&value=${sco.value}">${status.index}</a>
						</li>
					</c:forEach>
				</c:if>
			...
			<c:if test="${pagevo.pageNo < pagevo.totalPageSize}">
				<li><a href="${pagevo.url}type=search2&pageNo=${pagevo.pageNo + 1}&name=${sco.emp.name}&grade=${sco.grade}&project=${sco.pro.name}&department=${sco.dep.name}&value=${sco.value}">下一页</a></li>
				<li><a href="${pagevo.url}type=search2&pageNo=${pagevo.totalPageSize}&name=${sco.emp.name}&grade=${sco.grade}&project=${sco.pro.name}&department=${sco.dep.name}&value=${sco.value}">尾页</a></li>
			</c:if>
		</ul>
